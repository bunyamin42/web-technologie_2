...
<body>
    <nav>
        ...
        <div class="nav-right">
            ...
            <div class="nav-profil-icon">
                <img src="../images/profil_icon.png">
            </div>
            <div class="logout">
                <a href="php_dateien/logout.php" onmouseover="showLogoutText()" onmouseout="hideLogoutText()">
                    <span id="logoutText" class="logout-text">Abmelden</span>
                    <img src="../images/abmelden_icon.png">
                </a>
            </div>
    </nav>
    ...
    for (var i = 0; i < beiträge.length; i++) {
        ...
        // Rufe die Like-Anzahl für jeden Beitrag ab und aktualisiere die Anzeige
        zeigeLikeAnzahl(beitragElement, function(anzahlLikes) {
            beitragElement.innerHTML =
                "<p> <img src='../images/profil_icon.png'> " + beitrag.Benutzer + "</p>" +
                "<p>Beitrag: " + beitrag.Inhalt + "</p>" +
                "<div class='aktionen'>" +
                "<button onclick='liken(this)'>Liken</button> " +
                "<span class='anzahl-likes'>" + anzahlLikes + " Likes</span> " +
                "<button onclick='klappeKommentare(this)'>Kommentare</button>" +
                "</div>" +
                "<div class='kommentar-bereich'>" +
                "<textarea placeholder='Schreibe einen Kommentar...'></textarea>" +
...
function sendeLikeAnServer(beitrag) {
        ...
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                ...
                if (xhr.responseText === "success") {
                    ...
                    console.error("Fehler beim Speichern des Likes.");
                }
            } else {
                // Fehler beim Senden der Anfrage
                console.error("Fehler beim Senden der Like-Anfrage.");
            }
        }
    };
    ...
    // Sende die Daten an das PHP-Skript
    xhr.send("benutzername=" + encodeURIComponent(benutzer_id) + "&contentBeitrag=" + encodeURIComponent(contentBeitrag));
}
 ...
 function beitragGeliked(beitrag, callback) {
    ...
    // Funktion zum Abrufen der Like-Anzahl für einen Beitrag
    function zeigeLikeAnzahl(beitrag, callback) {
        var contentBeitrag = beitrag.getAttribute("title");

        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                var anzahlLikes = parseInt(xhr.responseText);
                callback(anzahlLikes);
            }
        };

        xhr.open("POST", "like_anzahl_anzeigen.php", true);
        xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhr.send("contentBeitrag=" + encodeURIComponent(contentBeitrag));
    }
